...
class LibraryServiceTests extends FlatSpec with MockitoSyrup with ScalaFutures with FailHelper {
  ...
  class TestFixture extends TimeSupport {
    ...
    val TestCatalogueInfo = CatalogueInfo(ISBN, "Title", "Sortable Title", "Author", coverImageLink.url, sampleEpubLink.url)
    val TestBookDetails = BookDetails(ISBN, TestCatalogueInfo.title, TestCatalogueInfo.sortTitle, TestCatalogueInfo.author, clock.now(), Full, ReadingStatus, Progress, images, links)
